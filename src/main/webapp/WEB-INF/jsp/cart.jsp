    ...
    <body class="hideresponsive">
                    ...
                    <div class="container">
                        <div class="row" id="cart">
                            <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 pd5">
                                            ...
                                            <c:forEach var="item" items="${cart.cart}">
                                                <tr id="delete-item${item.id}">
                                                    ...
                                                    <td class="qty">
                                                        ...
                                                        <input type="number" class="form-control" name="updates[]" min="1" <c:forEach var="max" items="${max}">
                                                                   <c:if test="${max.id==item.id}">max="${max.productQuantity}"</c:if>
                                                               </c:forEach> id="quantity${item.id}" value="${item.productQuantity}" oninput="updateQuantity(${item.id})">

                                                    </td>
                                                    <td class="price">${item.color.productColor}</td>
                                                    <td>${item.productSize.productSize}</td>
                                                    <td class="remove">
                                                        <a href="#" class="cart" id="delete-product-cart2" onclick="deleteId(${item.id})">
                                ...
                                <div class="clearfix">
                                    <a class="continue-shopping" title="Mua tiếp" href="home">Mua tiếp</a>
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 pd5">
                                ...
                                <div class="right-cart">
                                    <h2>
                                        ...
                                        <c:if test="${cart.cart.size()>= 1}"><label id="total-price"><fmt:formatNumber type = "number" 
        ...
        <script>
            function deleteId(itemId) {
                var xhttp;
                var divName = "delete-item" + itemId;
                xhttp = new XMLHttpRequest();
                xhttp.open("GET", "delete-item?id=" + itemId, true);
                xhttp.send();
            ...
            function updateQuantity(itemId) {
                ...
                var max = parseInt(document.getElementById("quantity" + itemId).max);
                var min = parseInt(document.getElementById("quantity" + itemId).min);
                if (parseInt(quantity.value) < min) {
                    quantity.value = min;
                    console.log(min);
                } else if (parseInt(quantity.value) > max) {
                    quantity.value = max;
                    console.log(max);
                }
                ...
                var xhttp;
                xhttp = new XMLHttpRequest();
                xhttp.open("GET", "update-quantity?id=" + itemId + "&quantity=" + qty, true);
                xhttp.send();
                xhttp.onreadystatechange = function () {
                    if (this.readyState === 4 && this.status === 200) {
                        var data = JSON.parse(this.responseText);
